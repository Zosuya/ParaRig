...
2. 按下 **Generate Slider Rig（生成滑桿綁定）**。滑桿依 `group` 欄位分組——每個分組對應一個外框（Frame）——所有生成物件都會統一放進專屬的「ParaRig」collection。改完清單內容後重新按一次 Generate 是非破壞性的：會就地更新，不會重置你已經調整過的把手位置或外框變換。
3. 沿著鎖定的軸向拖拽滑桿的圓形把手，驅動綁定的數值。
4. 按下 **Clear Generated Sliders（清除已生成的滑桿）** 可以整批移除重新開始。
